<template>
  <v-card>
    ...
    <div class="pl-5 pr-5">
      <v-row>
        <v-col cols="4">
         <v-select placeholder="Select Filter" style="width: 200px" v-model="filterType" :items="filterItems"></v-select> 
        </v-col>
          <v-col cols="4">
         <v-text-field style="width: 200px" v-model="search" @keyup="onSearch" placeholder="Search"></v-text-field>
          </v-col>
        <v-col cols="2">
...
export default {
  ...
  methods: {
    onSearch() {
      this.getProducts({ skip: this.skip, value: this.search, key: this.filterType})
    },
    onNext() {
